      ...
      <div className="min-h-screen bg-slate-50 flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center px-4">
          <div className="card p-8 text-center max-w-md">
            ...
            <h2 className="text-lg font-semibold text-slate-800 mb-2">No career match found</h2>
            <p className="text-sm text-slate-500 mb-4">Take the assessment first to see your skill gap analysis.</p>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="mb-6">
            ...
            <p className="mt-2 text-slate-500">
              Compare your current skill levels to what employers expect for <span className="font-semibold text-primary-600">{career?.title}</span>.
            </p>
          </div>
              ...
              return (
                <div key={gap.skill} className="card p-5">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <Badge variant={config.variant}>{gap.level}</Badge>
                  </div>
                  ...
                  {gap.gap > 0 && (
                    <p className="text-xs text-error-600 mt-2.5">
                      Gap of {gap.gap}% — you need to improve this skill.
                    </p>
                  )}
                  {gap.gap <= 0 && (
                    <p className="text-xs text-success-600 mt-2.5">
                      You meet or exceed the required level.
                    </p>
                  )}
                </div>
              );
              ...
              {biggestGaps.map((gap, i) => (
                <div key={gap.skill} className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-slate-500">Gap of {gap.gap}% to reach required level</p>
                  </div>
                  <Badge variant="error">{gap.level}</Badge>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="text-center">
            <Link to="/app/roadmap" className="btn-primary text-base px-6 py-3">
