    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="text-center mb-10">
          <h1 className="text-3xl lg:text-4xl font-bold text-slate-900">{t.opportunities.title}</h1>
          <p className="mt-3 text-slate-600 max-w-xl mx-auto">{t.opportunities.subtitle}</p>
        </div>
          ...
          {opportunities.map((opp) => (
            <div key={opp.id} className="card p-5 hover:shadow-lg transition-shadow flex flex-col">
              <div className="flex items-center gap-2 mb-3">
                ...
                <Badge variant={opp.match >= 90 ? 'success' : 'warning'}>{opp.match}% {t.opportunities.matchLabel}</Badge>
              </div>
